...
struct ccs_wrapper : sc_module {
	...
	sc_in<sc_logic>  act_port_vld;
	sc_in<sc_lv<256> >  act_port_dat;
	...
	ccs_wrapper(sc_module_name modelName): sc_module(modelName)
	{
		...
		SC_METHOD(ccs_wrapper_act_port_vld_action); sensitive << act_port_vld; snps_sysc_mark_last_create_process_as_internal();
		SC_METHOD(ccs_wrapper_act_port_dat_action); sensitive << act_port_dat; snps_sysc_mark_last_create_process_as_internal();
		...
		SC_METHOD(ccs_wrapper_act_port_rdy_action); sensitive << *sig_act_port_rdy; snps_sysc_mark_last_create_process_as_internal();
		...
		act_port_vld_string += "_R_act_port_vld";
		alterString(act_port_vld_string, basename());
		hdl_connect_v_int(*sig_act_port_vld, act_port_vld_string.c_str(), HDL_OUTPUT);

		std::string act_port_dat_string = name();
		act_port_dat_string += "_R_act_port_dat";
		alterString(act_port_dat_string, basename());
		hdl_connect_v_int(*sig_act_port_dat, act_port_dat_string.c_str(), HDL_OUTPUT);
		...
		act_port_rdy_string += ".act_port_rdy";
		hdl_connect_v_int(*sig_act_port_rdy, act_port_rdy_string.c_str(), HDL_INPUT);
		...
		vcsInstance->addPort("act_port_vld",  VcsPort::INPUT_PORT, 1, 0, "sc_logic", VcsPort::BC_C);
		vcsInstance->addPort("act_port_dat",  VcsPort::INPUT_PORT, 256, 0, "sc_lv", VcsPort::BC_C);
		...
		vcsInstance->addPort("act_port_rdy",  VcsPort::OUTPUT_PORT, 1, 0, "sc_logic", VcsPort::BC_C);
	...
	{ sig_act_port_vld->write(act_port_vld.read());
	}
	void ccs_wrapper_act_port_dat_action()
	{ sig_act_port_dat->write(act_port_dat.read());
	}
	...
	{ act_port_rdy.write((*sig_act_port_rdy).read());
	}
