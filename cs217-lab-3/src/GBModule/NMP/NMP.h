...
#pragma hls_unroll yes
  ...
  // Note that the I/O data is in adpfloat format, but computation is done
  // in fixed-point format.
  // ===========================================================================

  /**
   * @brief Convert input adpfloat vector to fixed-point format for computation.
   *
   * There is a bug in AdpfloatSpec.h that causes direct conversion from
   * adpfloat to fixed-point to fail during HLS using FPGA libraries. To work
   * around this, we first convert adpfloat to ac_float, then from ac_float to
   * fixed-point.
   */
  void ConvertInputToFixed() {
    spec::AdpfloatBiasType adpbias = nmp_config.adpbias_1;
#pragma hls_unroll yes
    for (int i = 0; i < spec::kVectorSize; i++) {
      spec::NMP::AdpType in_adp(large_rsp_reg.read_vector[0][i]);
      // Convert to ac_float then to fixed-point
      spec::NMP::ACFloatType in_float = in_adp.to_ac_float(adpbias);
      // Assign to fixed-point register
      input_fixed[i] = spec::NMP::FixedType(in_float.to_ac_fixed());
    }
  } // ConvertInputToFixed

  /**
   * @brief Convert fixed-point output back to adpfloat for write
   *
   * Again, due to a bug in AdpfloatSpec.h, we convert fixed-point to
   * ac_float first, then to adpfloat.
   */
  void ConvertOutputToAdpfloat() {
    spec::AdpfloatBiasType adpbias = nmp_config.adpbias_1;
#pragma hls_unroll yes
    for (int i = 0; i < spec::kVectorSize; i++) {
      spec::NMP::ACFloatType out_float = output_fixed[i];
      spec::NMP::AdpType out_adp;
      out_adp.set_value_ac_float(out_float, adpbias);
      write_data[i] = out_adp.to_rawbits();
    }
  } // ConvertOutputToAdpfloat
...
#pragma hls_unroll yes
    for (int i = 0; i < spec::kVectorSize; i++) {
      sum_sq += input_fixed[i] * input_fixed[i];
    }
    // CODE ENDS HERE ==========================================================
  ...
  void ComputeRMSSqrtRecip() {
    ...
    // CODE STARTS HERE ========================================================
    // Convert sum_sq to unsigned (it's always positive as it's sum of squares)
    spec::NMP::UnsignedAccumType sum_sq_unsigned = sum_sq;
    // Calculate mean of squares using the inverse vector size constant
    spec::NMP::UnsignedAccumType mean_sq = sum_sq_unsigned * spec::NMP::kInvVectorSize;
    // Add epsilon for numerical stability
    spec::NMP::UnsignedAccumType variance = mean_sq + spec::NMP::kEpsilon;
    // Compute sqrt
    spec::NMP::UnsignedAccumType rms_unsigned;
    ac_math::ac_sqrt_pwl(variance, rms_unsigned);
    // Convert to signed for reciprocal computation
    spec::NMP::AccumType rms = rms_unsigned;
    // Compute reciprocal
    ac_math::ac_reciprocal_pwl(rms, rms_reciprocal);
    // CODE ENDS HERE ==========================================================
...
#pragma hls_unroll yes
    for (int i = 0; i < spec::kVectorSize; i++) {
      ...
      ac_math::ac_exp_pwl(shifted[i], exp_values[i]);
    }
    // CODE ENDS HERE ==========================================================
